...
- 复用上一份审阅的中文时：用 `old_en` 或同一 `id` 套中文，英文已经改了，中文还是旧句。应只按当前 `en` 完全一致才复用；对不上的必须重译。删段才用 `old_en` 的旧译。章节重排后编号会变，禁止按 `id` 复用。
...
- 不编造旧版没有的句子来「对齐」。
- 译文忠实，不借翻译改技术含义。
- 复用旧译只按当前 `en` 完全一致；对不上必须重译。删段才用 `old_en` 的旧译。禁止用同一 `id` 或 `old_en` 给已经改过的英文套旧中文。
